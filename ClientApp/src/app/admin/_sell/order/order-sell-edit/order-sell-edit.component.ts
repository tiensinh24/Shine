...
import { OrderSellDetail } from "src/app/_shared/intefaces/sell/order/order-sell-detail";
import { MatTableDataSource } from "@angular/material/table";
import { OrderSellProducts } from "src/app/_shared/intefaces/sell/order/order-sell-products";
...
import { OrderSellEditDialogComponent } from "src/app/_shared/components/_sell/orders/order-sell-edit-dialog/order-sell-edit-dialog.component";
...
@Component({
  ...
  templateUrl: "./order-sell-edit.component.html",
  styleUrls: ["./order-sell-edit.component.scss"]
})
export class OrderSellEditComponent implements OnInit, OnDestroy {
  // Subsctiptions
  ...
  // Variables
  order: OrderSellDetail;

  // Table data source
  valueDataSource = new MatTableDataSource<OrderSellProducts>();
  ...
  constructor(
    private orderService: OrderSellService,
  ...
  private refreshOrder(order: OrderSell) {
  ...
  addEditLineItem(lineItem?: OrderSellProducts) {
    const dialogConfig = <MatDialogConfig>{
      disableClose: true,
      autoFocus: true,
      maxWidth: "100vw",
  ...
  deleteLineItem(lineItem: OrderSellProducts) {
    ...
    dialogRef.afterClosed().subscribe((res: boolean) => {
      if (res) {
        this.sub$.add(
          this.orderService
            .deleteOrderProduct(lineItem.orderId, lineItem.productId)
            .subscribe((res: boolean) => {
              if (res) {
                ...
                this.snackBar.open(
                  `Can't delete line item, please try again`,
                  "Success"
                );
              }
            })
  ...
  private refreshOrderValue() {
    this.order.orderTotal = this.order.products.reduce(
      (a, b) => a + b.total,
      0
    );
      ...
      if (res !== "cancel") {
        if (res) {
          ...
          if (cost === undefined) {
            ...
            const index = this.order.costs.findIndex(
              c => c.costId === res.costId
            );
            this.order.costs.splice(index, 1, res);

            this.snackBar.open("Cost has been updated", "Success");
          }
        ...
        this.sub$.add(
          this.costService.deleteCost(cost.costId).subscribe((res: boolean) => {
            if (res) {
              ...
              this.snackBar.open(
                `Can't delete cost, please try again`,
                "Error"
              );
            }
          })
      ...
      if (res !== "cancel") {
        if (res) {
          ...
          if (payment === undefined) {
            ...
            const index = this.order.payments.findIndex(
              c => c.paymentId === res.paymentId
            );
          ...
          this.paymentService
            ...
            .subscribe((res: boolean) => {
              if (res) {
                ...
                this.snackBar.open(
                  `Can't delete payment, please try again`,
                  "Error"
                );
              }
            })
  ...
  private refreshTotalPayment() {
    this.order.paymentTotal = this.order.payments.reduce(
      (a, b) => a + b.amount,
      0
    );
    this.refreshDebt();
  ...
  private refreshPieChart() {
    this.pieChartData = Object.assign(
      [[]],
      [["Value", this.order.orderTotal], ["Cost", this.order.costTotal]]
    );
  }
}
